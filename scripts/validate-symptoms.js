...
const symptoms = JSON.parse(
  readFileSync("./../src/data/symptomList.json", "utf-8"),
);
const synonyms = JSON.parse(
  readFileSync("./../src/data/synonymsSymptomList.json", "utf-8"),
);
...
if (missing.length > 0) {
  ...
  console.log(
    "✓ Tous les symptômes de db.json sont présents dans symptomList.json",
  );
}

// Vérification de la structure des synonymes
console.log(
  "✓ Structure des synonymes: clés principales uniquement (unidirectionnel)",
);
let synonymCount = 0;
